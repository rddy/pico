...
class AutoEncoder(nn.Module):
    ...
    def forward(self, x, mask=None, temp=None, n_masked_layers=None):
        s = self.stem(2 * x - 1.0)
        ...
        torched_mask = torch.from_numpy(mask[np.newaxis, np.newaxis, :, :]).float().contiguous().to('cuda')
        def prune(x, y=0):
            scaled_mask = torch.repeat_interleave(torched_mask, x.shape[2] // torched_mask.shape[2], dim=2)
            scaled_mask = torch.repeat_interleave(scaled_mask, x.shape[3] // scaled_mask.shape[3], dim=3)
            return scaled_mask * x + (1 - scaled_mask) * y
        ...
        mu_q = prune(mu_q)
        log_sig_q = prune(log_sig_q)
        dist = Normal(mu_q, log_sig_q, temp)   # for the first approx. posterior
        z, _ = dist.sample()
        ...
        for cell in self.dec_tower:
            if cell.cell_type == 'combiner_dec':
                if idx_dec > 0:
                    ...
                    dist_args = []
                    if temp is not None:
                        dist_args.append(temp)
                    unmasked = idx_dec < n_masked_layers if n_masked_layers is not None else True
                    if unmasked:
                        ...
                        mu_q = prune(mu_q)
                        log_sig_q = prune(log_sig_q)
                        dist = Normal(mu_p + mu_q, log_sig_p + log_sig_q, *dist_args) if self.res_dist else Normal(mu_q, log_sig_q)
                    else:
                        dist = Normal(mu_p, log_sig_p, *dist_args)
                    z, _ = dist.sample()
                    log_q_conv = dist.log_p(z)
                    # apply NF
                    if unmasked:
