...
class CarRacing(gym.Env, EzPickle):
    ...
    def step(self, action):
        ...
        info = {
          'succ': self.tile_visited_count > self.prev_tile_visited_count,
          'crash': self.off_road
        }

        car_x, car_y = self.car.hull.position
        car_pos = np.array([car_x, car_y])[np.newaxis, :]
        road_pos = np.array([[x[0][0], x[0][1]] for x, _ in self.road_poly])
        dists = np.linalg.norm(car_pos - road_pos, axis=1)
